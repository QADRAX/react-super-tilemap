 ...
 */
export function useComputedValues(state: ContextState, props: TilemapProps): ContextComputedState {
  ...
  const cameraAbsolutePosition = useMemo(() => {
    ...
    return getCameraPositionByTilePosition(state.cameraPosition, tileSize, state.canvasSize);
