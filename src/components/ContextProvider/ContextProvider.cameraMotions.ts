...
import { TilemapActions, _setCameraMotionQueue, _setCurrentCameraMotion } from "../../Context/TilemapContext.actions";
import { CameraMotion } from "../../types/Motions";
import { ContextState } from "../../types/TilemapContext";
import { getDistance } from "../../utils";

export function syncCameraMotions(
    ...
    useEffect(() => {
        if (!isCameraDragging && !currentCameraMotion && cameraMotionQueue.length > 0 && cameraPosition) {
            ...
            const speed = nextMotionRequest.settings.speed > 0 ? Math.abs(nextMotionRequest.settings.speed) : 1;
