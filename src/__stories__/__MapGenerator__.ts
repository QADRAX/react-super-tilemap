...
export function getRandomSpriteSchemaWithLayers(rows: number, cols: number): string[][][] {
  const schema: string[][][] = [];
...
export function getRandomSpriteSchema(rows: number, cols: number): string[][][] {
  const schema: string[][][] = [];
  for (let i = 0; i < rows; i++) {
    const row: string[][] = [];
    for (let j = 0; j < cols; j++) {
...
export function getFullfilledSchema(
  ...
  cols: number,
  sprite: string,
  numLayers: number = 1 // eslint-disable-line @typescript-eslint/no-inferrable-types
): string[][][] {
  const schema: string[][][] = [];
  for (let i = 0; i < rows; i++) {
    const row: string[][] = [];
